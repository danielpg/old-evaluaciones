 ...
 * @return mixed
 */
	function cdispatchMethod($model, $method, $params = array()) {
		if (empty($params)) {
 ...
 * @access public
 */
	function cdispatchMethod(&$model, $method, $params = array(), $strict = false) {
		...
		if (!empty($call)) {
			return $this->{$call[1]}->cdispatchMethod($model, $call[0], $params);
		}
 ...
 */
	function trigger(&$model, $callback, $params = array(), $options = array()) {
		...
		for ($i = 0; $i < $count; $i++) {
			...
			$result = $this->{$name}->cdispatchMethod($model, $callback, $params);
