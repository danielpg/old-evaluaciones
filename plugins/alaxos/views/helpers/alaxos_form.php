...
class AlaxosFormHelper extends FormHelper
{
	...
	function filter_boolean($fieldname, $attributes = array())
	{
	    ...
	    {
	        $attributes['options'] = array('1' => __( 'yes', true), '0' => __( 'no', true));
	    }
	...
	public function input_true_false($fieldname, $options = array(), $selected = null, $attributes = array())
	{
	    if(empty($options))
	    {
	        $options = array('1' => __('true', true), '0' => __( 'false', true));
	    }
	...
	public function input_actions_list($fieldName = '_Tech.action', $options = array())
	{
	    ...
	    $actions       = !empty($options['actions']) ? $options['actions'] : array('deleteAll' => __( 'delete all', true));
	    
	    /*
	     * Include translated texts for JS confirm box
	     */
	    $script  = 'var confirmDeleteAllText =            "' . __( 'are you sure you want to delete all those items ?', true) . '";' . "\n";
	    $script .= 'var pleaseChooseActionToPerformText = "' . __( 'please choose the action to perform', true) . '";' . "\n";
