...
<?php
	//echo "\t<?php\n";
 	...
 	echo "\t<?php\n";
    echo "\techo \$this->AlaxosForm->create('{$modelClass}', array('url' => \$this->passedArgs)); //'url' => \$this->passedArgs allows to keep the sort when searching\n";
    echo "\t?>\n";
    ?>
...
//	}
	?>
		<td class="searchHeader" style="width:80px">
    		...
    		<?php
    		echo "<?php\n";
    		echo "\t\t\techo \$this->AlaxosForm->create('{$modelClass}', array('id' => 'chooseActionForm', 'url' => array('controller' => '{$pluralVar}', 'action' => 'actionAll')));\n";
    		echo "\t\t\t?>\n";
    		?>
    	</td>
	</tr>
	...
	echo "\t\t<td>\n";
	echo "\t\t<?php\n";
	echo "\t\techo \$this->AlaxosForm->checkBox('{$modelClass}.' . \$i . '.id', array('value' => \${$singularVar}['{$modelClass}']['id']));\n";
	echo "\t\t?>\n";
	echo "\t\t</td>\n";
		...
		{
		    echo "\t\t\t<?php echo \$this->Html->link(\$this->Html->image('/alaxos/img/toolbar/small_drop.png'), array('action' => 'delete', \${$singularVar}['{$modelClass}']['id']), array('escape' => false), sprintf(___(\"are you sure you want to delete '%s' ?\", true), \${$singularVar}['{$modelClass}']['{$displayField}'])); ?>\n";
		}
		else
		{
		    echo "\t\t\t<?php echo \$this->Html->link(\$this->Html->image('/alaxos/img/toolbar/small_drop.png'), array('action' => 'delete', \${$singularVar}['{$modelClass}']['id']), array('escape' => false), sprintf(___(\"are you sure you want to delete this element ?\", true))); ?>\n";
		}
	...
	echo "\techo '&nbsp;';\n";
	echo "\techo \$this->AlaxosForm->end(array('label' =>___d('alaxos', 'go', true), 'div' => false));\n";
	echo "\techo '</div>';\n";
	echo "}\n";
	echo "?>\n";
	?>
